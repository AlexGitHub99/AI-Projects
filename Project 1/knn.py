...
def main():
    ...
    for i in range(len(example_train_x)):
        assert([i] == get_nearest_neighbors(example_train_x, example_train_x[i], 1)[0])
        
    #########
    # Sanity Check 2: See if neighbors are right for some examples (ignoring order)
    nn_idx = get_nearest_neighbors(example_train_x, np.array( [ 1, 4, 2] ), 2)[0]
    assert(set(nn_idx).difference(set([4,3]))==set())

    nn_idx = get_nearest_neighbors(example_train_x, np.array( [ 1, -4, 2] ), 3)[0]
    assert(set(nn_idx).difference(set([1,0,2]))==set())

    nn_idx = get_nearest_neighbors(example_train_x, np.array( [ 10, 40, 20] ), 5)[0]
    ...
    p_nn_idx = get_nearest_neighbors(example_train_x, query, 1)[0]
    for k in range(2,7):
      nn_idx = get_nearest_neighbors(example_train_x, query, k)[0]
    ...
    for k in [1, 3, 5, 7, 9, 99, 999, 8000]:
        ...
        #######################################
        predicted_y = predict(train_X, train_y, train_X, k, s)
        train_acc = compute_accuracy(train_y, predicted_y)
        
        #######################################
        # TODO Compute 4-fold cross validation accuracy
        #######################################
        val_acc, val_acc_var = cross_validation(train_X, train_y, 4, k, s)
    ...
    pred_test_y = predict(train_X, train_y, test_X, best_k, 10)    
...
def get_nearest_neighbors(example_set, query, k):
    ...
    dist_set = np.linalg.norm(example_set - query, 2, 1)
    d_i_list = heapq.nsmallest(k, enumerate(dist_set), key=lambda x: x[1])
    idx_of_nearest = [i for i, _ in d_i_list]
    return idx_of_nearest, dist_set 
...
def knn_classify_point(examples_X, examples_y, query, k, s):
    #TODO
    sigma = s

    idx_of_nearest, dist_set = get_nearest_neighbors(examples_X, query, k)
    success_count = sum([examples_y[i][0] for i in idx_of_nearest])
    predicted_label = int(success_count / k > 0.5)
    # weight_sum_yes = sum([examples_y[i][0]*weight(dist_set[i], sigma) for i in idx_of_nearest])
    # weight_sum_no = sum([(1-examples_y[i][0])*weight(dist_set[i], sigma) for i in idx_of_nearest])
    # predicted_label = int(weight_sum_yes > weight_sum_no)

    return predicted_label

#weight function
def weight(dist, sigma):
...
def cross_validation(train_X, train_y, num_folds=4, k=1, s=10):
    #TODO

    train_X_split = np.split(train_X, num_folds)
    train_y_split = np.split(train_y, num_folds)
    accuracies = []
    for i in range(num_folds):
        training_X = np.vstack([x for j, x in enumerate(train_X_split) if i != j])
        training_y = np.vstack([x for j, x in enumerate(train_y_split) if i != j])
        predicted_y = predict(training_X, training_y, train_X_split[i], k, s)
        accuracies.append(compute_accuracy(train_y_split[i], predicted_y))

    avg_val_acc = np.mean(accuracies)
    varr_val_acc = (sum([(x - avg_val_acc)**2 for x in accuracies])/num_folds)
...
def predict(examples_X, examples_y, queries_X, k, s): 
    # For each query, run a knn classifier
    predicted_y = [knn_classify_point(examples_X, examples_y, query, k, s) for query in queries_X]
